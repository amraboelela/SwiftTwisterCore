...
open class TwisterCore {
    ...
    open func postsFor(username: String) -> String? {
        ...
        twisterCoreGetPosts(username.cString, username.count, 20, &rawData, &rawDataLength)
        ...
        /*var rawData: UnsafeMutableRawPointer? = nil
         var rawDataLength: Int = 0
         twisterCoreGetPosts(char *username, int usernameLength, int max, void **result, int *resultLength);
         //let status = levelDBItemGet(db, key.cString, key.count, &rawData, &rawDataLength)
         if status != 0 {
         return
         }
